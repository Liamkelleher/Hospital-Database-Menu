...
Purpose: 
A program designed for manipulating records of patients and employees in a hospital. The program allows any user to query the data and to obtain some summary information. 6 options are presented to the user, of which display unique information regarding employees and patients of the hospital. See "How to use the software) for more about functionality.
...
While the menu option is displaying, if a user enters a non-integer, the program enters an endless loop.
For displaying the summary of patients by department and employees by position, my implementation is not the most memory efficient. I create multiple variables for each dept/position and switch cases for each. These make for a messy approach to the problem, and heavy duplication of code.
